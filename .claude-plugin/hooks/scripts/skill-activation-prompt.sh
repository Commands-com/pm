...
echo "$(date): Raw hook_data: $hook_data" >> /tmp/skill-activation-debug.log
...
alt_prompt=$(echo "$hook_data" | jq -r '.prompt // .message // .text // empty')
echo "$(date): Alternative extraction: $alt_prompt" >> /tmp/skill-activation-debug.log
